...
async function loadVideos(){
    ...
    function fetchPersonalizedManifest(video, ctx){
        const { player, stage, wrapper, viewer } = ctx;
        ...
        async function poll(){
            ...
            try{
                ...
                if(res.ok){
                    ...
                    let personalizedUrl = (data && data.personalizedUrl) ? String(data.personalizedUrl) : 
                        (data && data.url) ? String(data.url) : 
                        (video.personalizedUrl) ? String(video.personalizedUrl) : 
                        apiBase + '/media/' + encodeURIComponent(video.id) + '?expires=' + encodeURIComponent(parsed.expires) + '&signature=' + encodeURIComponent(parsed.signature) + '&personalized=1' + (_accessToken ? '&token=' + encodeURIComponent(_accessToken) : '');
                    ...
                    if(cacheHeader) wrapper.setAttribute('data-personalized-cache','HIT');
                    ...
                    if(finalSrc && isHit && stripBase(curSrc) !== stripBase(finalSrc)){
    ...
    function svgPosterDataUrl(video){
        const svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 360" preserveAspectRatio="xMidYMid slice"><rect width="640" height="360" fill="#0a0a0c"/><g opacity="0.55" stroke="#b3b3bd" stroke-width="6" fill="none" stroke-linecap="round" stroke-linejoin="round"><rect x="230" y="135" width="140" height="90" rx="14"/><path d="M370 168l44 -22v68l-44 -22z"/></g></svg>';
        try{ return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg); }catch(e){ return ''; }
    }
    ...
    async function renderVideos(){
        ...
        try{
            ...
            if(whoRes.ok){
                ...
                cachedWhoamiViewer = whoData.user ? (whoData.user.global_name || whoData.user.username) : null;
            }
        }catch(e){}
        ...
        filteredVideos.forEach(function(video, videoIdx){
            ...
            if(video.thumbnail){
                ...
                thumb.onerror = function(){ posterHost.replaceChildren(createThumbnailFallback()); };
        ...
        function openVideoModalAt(idx){
            ...
            if(!modalObj) return;
            const { modal, stage, player, loader, titleEl, metaEl } = modalObj;
            ...
            const cleanup = fetchPersonalizedManifest(video, {
                player, stage, wrapper: modal, 
                viewer: (video.viewer || cachedWhoamiViewer || 'viewer')
            });
            modal._cancelPoll = cleanup;
            
            function onCanPlay(){ 
                if(loader) loader.classList.add('hidden'); 
                player.removeEventListener('canplay', onCanPlay); 
                player.removeEventListener('loadeddata', onCanPlay); 
            }
